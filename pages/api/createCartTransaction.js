...
const createTransaction = async (req, res) => {
  ...
  //  for each id in ids array query graphcms for the price of the product
    const query = gql`
      query {
        ${ids.map((id) => {
          return `product${id}: products(where: {id: "${id}"}) {
            id
            price
            owner
            type
    ...
    const prices = ids.map((id) => {
      return data[`product${id}`][0].price;
    });
    console.log("PRICES ARE THIS!!!", prices);
    const owners = ids.map((id) => {
      return data[`product${id}`][0].owner;
    });
    console.log("OWNERS ARE THIS!!!", owners);
