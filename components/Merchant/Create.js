...
const CreateProduct = () => {
  ...
  async function onChange(e) {
    ...
    try {
      console.log("add alert here please", files[0].name);
      // const added = await client.add(files[0]);
      setFile({ filename: files[0].name, hash: cid });
      // setNewProduct({ ...newProduct, collection: uploadWallet })
      alert('Product Grabbed, please finish info and click "CREATE PRODUCT".');
    } catch (error) {
